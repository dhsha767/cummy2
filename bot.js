...
function updateLeaderboard() {
  ...
  pgClient.query('select * from karma where lastmeme>=' + (new Date().getTime() - LEADERBOARD_MAX_TIME_SINCE_LAST_MEME) + ' and memes>=' + LEADERBOARD_MIN_MEMES + ' order by CAST((karmafrommemes/memes)-(downvotes/'+AAKPM_DOWNVOTE_COEFF+') as float) desc limit '+LEADERBOARD_MAX_COUNT+';').then(res => {
